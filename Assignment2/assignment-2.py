...
count_pairs_all = pd.read_sql(f"""
    ...
    """, conn)
print(count_pairs_all['pair_count'])
...
# __ANSWER:__ 
#
# The first design consideration would be to partition the data. We can partition by operating day, and partitioning by time for instance by hour will make it faster to run the query. Column-level pruning can further optimize performance. If most of the queries filter the data by a specific date range, day of operation, or even by weekday (in this case "monday = TRUE" for example or WHERE operating_day BETWEEN DATE 'X' AND DATE 'Y') we can partition the table on that column to drastically improve our queries and make the table more optimized. For example, including the operating day in the table schema, partitioning on that column lets the query engine skip entire partitions that aren’t in the filter range.
#
# We see that there are multiple trips that have different different `trip_id` but are otherwise exactly the same, assuming those are the same bus lines at the same time, but different days of the week. We can filter that information to make the dataset much smaller by deleting all the repeating rows and keeping only one trip to avoid storing redundant/unnecessary data.
#
# Implementing clustering indices, possibly by stop_id, weekday, or operating_day, would streamline queries as well. Clustering and sorting by relevant columns would also enhance query speed.
# -
