...
# Cache normalized stop mapping
spark.sql("""
    CREATE OR REPLACE TEMPORARY VIEW cached_stops AS
    SELECT 
        stop_id,
        LOWER(TRIM(stop_name)) AS norm_name
    FROM iceberg.sbb.stops
""")

# Materialize in memory 
spark.sql("CACHE TABLE cached_stops")


# Filter Trips with All Conditions

trips_TL_2024_df = spark.sql("""
    -- Join with cached stops and apply initial filters
    WITH filtered_trips AS (
        SELECT 
            i.trip_id,
            to_date(i.operating_day) AS operation_date
        FROM iceberg.sbb.istdaten i
        JOIN cached_stops s ON LOWER(TRIM(i.stop_name)) = s.norm_name
        WHERE 
            i.operator_abrv = 'TL' AND
            YEAR(to_date(i.operating_day)) = 2024
    ),
    
    -- Count operating days per trip
    trip_activity AS (
        SELECT 
            trip_id,
    ...
    -- output: trips with >=200 operating days
    SELECT trip_id
    FROM trip_activity
    WHERE active_days >= 200
""")

trips_TL_2024_df.cache()
trips_TL_2024_df.createOrReplaceTempView("trips_TL_2024_df")

# %%
trips_TL_2024_df.write.mode("overwrite").parquet(f"{hadoopFS}/user/{username}/assignment-3/data/trips_TL_2024_df")

# %%
# %%time
# regular count
trips_TL_2024_df.count()
...
'''
Lausanne circuits:  
Temporary storage for circuit ids
Get all distinct circuits from istdaten 
match them with trip ids
do a INNER join with trip ids from a.
You have circuits that overlap with in some degreee with Lauanne 


Main query 
extract: trip_ids, operatiod day, stops, dep/arrival time/actuall time
extract hidden: circuit id, unplanned, failed
Filter based on: 
    curcits that match Lauanne circuits
    year 2024
    un-failed/planned
'''

filtered_istdaten = spark.sql("""
    -- Identify all circuit_ids used by Lausanne trips
    WITH lausanne_circuits AS (
        SELECT DISTINCT i.circuit_id
        FROM iceberg.sbb.istdaten i
        INNER JOIN trips_TL_2024_df tl ON i.trip_id = tl.trip_id
        WHERE i.circuit_id IS NOT NULL
    )
    
    -- Get all valid trips sharing these circuits
    SELECT 
        i.operating_day,
        i.trip_id,
        i.bpuic,
        ...
        i.arr_actual
    FROM iceberg.sbb.istdaten i
    INNER JOIN lausanne_circuits lc ON i.circuit_id = lc.circuit_id
    WHERE 
        YEAR(to_date(i.operating_day)) = 2024 AND
        i.unplanned = FALSE AND  
        i.failed = FALSE         
""")
...
# %%
filtered_istdaten.write.mode("overwrite").parquet(f"{hadoopFS}/user/{username}/assignment-3/data/filtered_istdaten")

# %% [markdown]
# ![image.png](attachment:37c9fff7-cd75-465c-a66e-b015e7229b4e.png)
...
from pyspark.sql import Window
import pyspark.sql.functions as F
from pyspark.sql.functions import col, first, row_number, when

seq_window = Window.partitionBy("operating_day", "trip_id") \
.orderBy("arr_time", "arr_actual")

# Arr actual serves as the tie breaker

# %%
## TODO - create the table as described above

trip_sequences_df = (filtered_istdaten
    .withColumn("start_time", 
                F.first("dep_time", ignorenulls=True).over(seq_window))
                # First departure for each trip ignoring null 
    .withColumn("sequence", 
                F.row_number().over(seq_window))
                # assigning sequence number

    # Alter relative arrival/departure times with start_time (if NULL: leave)
    .withColumn("arr_time_rel",
                F.when(F.col("arr_time").isNotNull(),
                   F.expr("arr_time - start_time")).otherwise(None))
    .withColumn("arr_actual_rel",
                F.when(F.col("arr_actual").isNotNull(),
                   F.expr("arr_actual - start_time")).otherwise(None))
    .withColumn("dep_time_rel",
                F.when(F.col("dep_time").isNotNull(),
                   F.expr("dep_time - start_time")).otherwise(None))
    .withColumn("dep_actual_rel",
                F.when(F.col("dep_actual").isNotNull(),
                   F.expr("dep_actual - start_time")).otherwise(None))
    # create df based on schema req. 
    .select(
        "operating_day",
        "trip_id",
        "bpuic",
...
# %%
trip_sequences_df.write.mode("overwrite").parquet(f"{hadoopFS}/user/{username}/assignment-3/data/trip_sequences_df")
...
# %% [markdown]
# ### Loading dataframes from assignment 2
# * Data Source Selection and Exploration:
...
# %%
# %%time
trips_pivoted_df.select('bpuic','sequence','evt_type', "85:151:TL031-4506262507505612", "85:151:TL031-4506262507505612_2024-01-03", "85:151:TL031-4506262507505612_2024-01-04", "85:151:TL031-4506262507505612_2024-01-04").show()

# %%
# %%time
# 1. Calculate delays (actual - scheduled) for each observation
...
# %%
delay_flags_df.write.mode("overwrite").parquet(f"{hadoopFS}/user/{username}/assignment-3/data/delay_flags_df")
...
cols = [c for c in delay_flags_df.columns if c.startswith("delay_flag_")]

# Aggregate to get total count for all columns
results = delay_flags_df.agg(
    F.sum(sum(F.when(F.col(c).isNull(), 1).otherwise(0) for c in cols)).alias("total_nulls"),
    F.sum(sum(F.when(F.isnan(c) | F.col(c).eqNullSafe(float('nan')), 1).otherwise(0) for c in cols)).alias("total_nans"),  
    F.sum(sum(F.col(c) for c in cols)).alias("total_ones"),
    F.sum(sum(F.when(F.col(c).isNotNull(), 1).otherwise(0) for c in cols)).alias("total_digits")
).collect()[0]

print(f"""
Data Quality Report (delay_flags_df):
- NULL instances: {results['total_nulls']}
- NaN instances: {results['total_nans']}
- Significant delays (>5min): {results['total_ones']}
- Total valid binaries (0/1): {results['total_digits']}
""")


# %%
# get weather for the time period
# idea is to use this to compute a dataframe with daily average precepitation level, that can be merged with delay_flags_df @ day.
...
# %% [markdown]
# ### Feature Engineering (by stop by weekday)

# %%
# %%time
from pyspark.sql.functions import col, lit, when, expr, to_date, date_format, array, explode, struct, dayofweek
from pyspark.ml.feature import VectorAssembler

# Step 1: Extract (date, delay_flag) columns into rows for easier manipulation
date_cols = [col for col in delay_flags_df.columns if col.startswith("delay_flag_")]

# Create an array of structs (date, value), then explode
delay_flags_exploded = delay_flags_df.select(
    "bpuic",
    array([struct(
        lit(c.replace("delay_flag_", "")).alias("date"),
        col(c).alias("flag")
    ) for c in date_cols]).alias("date_flags")
).withColumn("date_flag", explode(col("date_flags"))).select(
    "bpuic",
    col("date_flag.date").alias("date"),
    col("date_flag.flag").alias("flag")
)

# Step 2: Add weekday
# (1 = Sunday, 2 = Monday, ..., 7 = Saturday)
# We'll shift so that Monday = 1, Sunday = 7 (to match ISO-8601)
delay_flags_weekday = delay_flags_exploded.withColumn(
    "weekday_raw", dayofweek(to_date("date"))
).withColumn(
    "weekday", ((col("weekday_raw") + 5) % 7) + 1  # Convert Sunday=1 → 7, Monday=2 → 1, ..., Saturday=7 → 6
).drop("weekday_raw")

# Step 3: Apply weight to flags using 1:15 imbalance ratio
# We'll treat class 1 (delayed) as weight 15, class 0 as weight 1
weighted_flags = delay_flags_weekday.withColumn(
    "weighted_flag", when(col("flag") == 1, 15).otherwise(1)
)

# Step 4: Aggregate per (bpuic, weekday)
agg_df = weighted_flags.groupBy("bpuic", "weekday").agg(
    expr("sum(weighted_flag) as delay_score")
)

# Step 5: Pivot weekdays to columns (to get a fixed-length vector per bpuic)
pivot_df = agg_df.groupBy("bpuic").pivot("weekday", list(range(1, 8))).sum("delay_score")

# Fill nulls with 0 (no delays recorded that weekday)
pivot_df = pivot_df.fillna(0)

# Step 6: Assemble into a feature vector
assembler = VectorAssembler(
    inputCols=[str(i) for i in range(1, 8)],  # Monday to Sunday
    outputCol="features"
)

feature_vector_df = assembler.transform(pivot_df).select("bpuic", "features")

# %%
feature_vector_df.write.mode("overwrite").parquet(f"{hadoopFS}/user/{username}/assignment-3/data/feature_vector_df")

# %%
feature_vector_df.show(truncate=False)

# delay score matrix: with stops as rows and weekdays as columns  
...
# +------------------------------+-----+
# |85:740:525-1                  |3141 |
# |85:741:TPN803-4503784319875957|4168 |
# |85:741:TPN804-4503784320450825|3274 |
# |85:741:TPN804-4503784320450915|2559 |
# |85:741:TPN805-4503784319876352|2327 |
# +------------------------------+-----+
...
# indexiing columns by categories
stop_idx = StringIndexer(inputCol="bpuic",outputCol="stop_idx",handleInvalid="keep")
trip_idx = StringIndexer(inputCol="trip_id", outputCol="trip_idx",handleInvalid="keep")
wx_idx= StringIndexer(inputCol="wx_phrase", outputCol="wx_idx",handleInvalid="keep")

ohe = OneHotEncoder(inputCols=["stop_idx", "trip_idx", "wx_idx"],outputCols=["stop_ohe", "trip_ohe", "wx_ohe"]) # one-hot encoding

# Numeric columns
numeric_cols = ["hist_delay_rate", "hist_obs", "temp", "rh", "precip_hrly",F.hour("start_time").alias("hour")] 

full_df = full_df.withColumn("hour", F.hour("start_time")).persist()
_ = full_df.count()

# Simple StringIndexers (very light shuffle)
stop_idx = StringIndexer(inputCol="bpuic",     outputCol="stop_idx", handleInvalid="keep")
trip_idx = StringIndexer(inputCol="trip_id",   outputCol="trip_idx", handleInvalid="keep")
wx_idx   = StringIndexer(inputCol="wx_phrase", outputCol="wx_idx",   handleInvalid="keep")

numeric_cols = ["hist_delay_rate", "hist_obs", "temp",
                "rh", "precip_hrly", "hour"]

assembler = VectorAssembler(inputCols=["stop_idx", "trip_idx", "wx_idx"] + numeric_cols,outputCol="features")


# %% jupyter={"source_hidden": true}
# grad-boosted tree – categorical indices
gbt = GBTClassifier(labelCol="delay_gt5min",featuresCol="features",maxIter=100,maxDepth=8)

pipe = Pipeline(stages=[stop_idx, trip_idx, wx_idx, assembler, gbt])

model = pipe.fit(full_df)    
